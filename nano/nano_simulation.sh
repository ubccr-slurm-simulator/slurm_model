...
confirm_proces(){
  ...
  for ((i=0; i < timeout_time; i++))
    {
        if grep -Fxq "${confirm_sentence}" "${RES_SAVE_DIR}"/"${CORE_COUNT}"/simulation.log; then
...
confirm_accounting_information
  if [ "${check_account_result}" == 0 ];then
    ...
    if [ ${index} == 0 ]; then
      ...
      RES_SAVE_DIR=${CUR_DIR}/profile_none
    elif [ ${index} == 1 ]; then
      mkdir -p ${CUR_DIR}/profile_10s/doc
      RES_SAVE_DIR=${CUR_DIR}/profile_10s
    elif [ ${index} == 2 ]; then
      mkdir -p ${CUR_DIR}/profile_20s/doc
      RES_SAVE_DIR=${CUR_DIR}/profile_20s
    else
      mkdir -p ${CUR_DIR}/profile_30s/doc
      RES_SAVE_DIR=${CUR_DIR}/profile_30s
    fi
    mkdir -p "${RES_SAVE_DIR}"/"${CORE_COUNT}"

    echo "${RES_SAVE_DIR}"/"${CORE_COUNT}"
    ...
    >& "${RES_SAVE_DIR}"/"${CORE_COUNT}"/"profile_io.out" &

    echo "Starting simulation"
    docker exec head-node python3 -u /slurm_model/apps/simulation/simjobs.py \
    /slurm_model/apps/simulation/job_traces/job_traces.csv >& "${RES_SAVE_DIR}"/"${CORE_COUNT}"/simulation.log &
    ...
    if [ "${check_result}" == 0 ];then
      ...
      --starttime 2020-05-01T00:00:00 --endtime 2020-12-30T00:00:00 >& "${RES_SAVE_DIR}"/"${CORE_COUNT}"/sacct.log &
      sleep 5
      ./vc_stop
    else
