...
read_sacct_out <- function(filename,nodes_desc=NULL,extract_node_list=FALSE, jobrec_prefix="jobid_"){
    cat("Read: ",filename,"\n")
    ...
    #convert job names
    if(sum(grepl("jobid_",slurm_log$JobName))==length(slurm_log$JobName)){
      slurm_log$JobRecID <- as.integer(str_replace(slurm_log$JobName, "jobid_",""))
    } else if(sum(grepl("job_",slurm_log$JobName))==length(slurm_log$JobName)){
      slurm_log$JobRecID <- as.integer(str_replace(slurm_log$JobName, "job_",""))
      
    } else {
      slurm_log$JobRecID <- as.integer(slurm_log$JobId)
    }
...
sacct2 <- bind_rows(
  ...
  read_sacct_out_multiple("sm", path.expand("~/slurm_sim_ws/slurm_model_archive/ubhpc/mightybee_sim/workload"), 
                          c(71, 241, 261), 1),
  read_sacct_out_multiple("sa10", path.expand("~/slurm_sim_ws/slurm_model_archive/ubhpc/anvil_sim/anvil10/workload"), 
                          c(37, 55,79, 137,234, 241, 261), 1),
  read_sacct_out_multiple("sa4free", path.expand("~/slurm_sim_ws/slurm_model_archive/ubhpc/anvil_sim/anvil4free/workload"), 
                          c(37, 55, 137,232), 1),
