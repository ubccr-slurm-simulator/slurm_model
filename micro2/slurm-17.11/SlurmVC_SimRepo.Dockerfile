...
RUN yum -y update && \
    ...
    yum -y install --setopt=tsflags=nodocs \
        ...
        openssl openssl-libs openssl-devel openssh openssh-clients openssh-server \
...
# install slurm
RUN git clone --depth 1  --branch slurm-17-11 https://github.com/ubccr-slurm-simulator/slurm_simulator.git && \
    cd ~/slurm_simulator && mkdir bld_frontend && cd bld_frontend && \
    ~/slurm_simulator/configure --prefix=/opt/slurm_front_end \
        --disable-x11 --enable-front-end --enable-pam --with-mysql-config=/usr/bin/&& \
    make -j install && \
    cd .. && mkdir bld && cd bld && \
    ~/slurm_simulator/configure --prefix=/opt/slurm \
        --disable-x11 --enable-pam --with-mysql-config=/usr/bin/ && \
    make -j install && \
...
COPY ./micro2/slurm-17.11/etc  /opt/cluster/micro2/etc
COPY ./micro2/slurm-17.11/utils /opt/cluster/micro2/utils
